...
(function () {
    ...
    const makeDialogBody = (urlInput, evoSearchItems, displayText, titleText, catalogs, hasUploadPanel, fileTypes) => {
    ...
    const makeDialog = (settings, onSubmit, editor) => {
        ...
        const initialData = Object.assign(
            getInitialData(settings, defaultTarget),
            {
                evo_search: '',
                evo_results: []
            }
        ...
        const buildBaseUrl = (base) => {
            let url = (base || '').toString().replace(/\/+$/, '');
            if (url && url[0] !== '/') {
                url = '/' + url;
            }
            return url;
        ...
        const defaultEvoSearchUrl = () => {
            const cfg = window.eTinyMCEConfig || {};
            const baseUrl = buildBaseUrl(cfg.baseUrl || '');
            return baseUrl + '/evo-link-search';
        };
        const evoDefaults = {
            enabled: true,
            searchUrl: defaultEvoSearchUrl(),
            minChars: 2,
            debounce: 250,
            limit: 10,
        ...
        const evoRaw = editor.getParam('evolinks', {});
        const evoSettings = Object.assign({}, evoDefaults, isObject(evoRaw) ? evoRaw : {});
        evoSettings.minChars = normalizeInt(evoSettings.minChars, evoDefaults.minChars);
        evoSettings.debounce = normalizeInt(evoSettings.debounce, evoDefaults.debounce);
        evoSettings.limit = normalizeInt(evoSettings.limit, evoDefaults.limit);
        evoSettings.cacheSize = normalizeInt(evoSettings.cacheSize, evoDefaults.cacheSize);
        evoSettings.includeUnpublished = normalizeBool(evoSettings.includeUnpublished, evoDefaults.includeUnpublished);
        evoSettings.includeHidden = normalizeBool(evoSettings.includeHidden, evoDefaults.includeHidden);
        if (!evoSettings.searchUrl) {
            evoSettings.searchUrl = evoDefaults.searchUrl;
        }
        const evoEnabled = evoSettings.enabled !== false && !!evoSettings.searchUrl;
        const evoState = {
            searchItems: [],
            searchMap: {},
            cache: {},
        ...
        const updateEvoCache = (key, value) => {
            ...
            if (!evoState.cache[key]) {
                evoState.cacheOrder.push(key);
            }
            evoState.cache[key] = value;
            if (evoState.cacheOrder.length > evoSettings.cacheSize) {
                const oldest = evoState.cacheOrder.shift();
                if (oldest) {
                    delete evoState.cache[oldest];
                }
            }
        };
        const buildEvoHref = (item) => {
            if (!item) {
                return '';
            }
            if (evoSettings.outputMode === 'relative') {
                if (item.uri) {
                    return ensureLeadingSlash(item.uri);
                }
                showEvoNotice('Missing URI for selected resource. Falling back to placeholder.');
            }
            else if (evoSettings.outputMode === 'absolute') {
                if (item.url) {
                    return item.url;
                }
                showEvoNotice('Missing URL for selected resource. Falling back to placeholder.');
            }
            return `[~${item.id}~]`;
        };
        const fetchEvoSearch = (query) => new Promise((resolve) => {
            if (!evoEnabled) {
                resolve([]);
                return;
            }
            if (evoState.cache[query]) {
                resolve(evoState.cache[query]);
                return;
            }
            const params = [
                'q=' + encodeURIComponent(query),
                'limit=' + encodeURIComponent(evoSettings.limit)
            ];
            if (evoSettings.includeUnpublished) {
                params.push('includeUnpublished=1');
            }
            if (evoSettings.includeHidden) {
                params.push('includeHidden=1');
            }
            const url = evoSettings.searchUrl + (evoSettings.searchUrl.indexOf('?') === -1 ? '?' : '&') + params.join('&');
            const handleData = (data) => {
                const items = Array.isArray(data) ? data : [];
                updateEvoCache(query, items);
                resolve(items);
            };
            ...
            if (tinymce?.util?.XHR?.send) {
                tinymce.util.XHR.send({
                    url,
                    success: handleText,
                    error: () => {
                        showEvoNotice('EVO search failed.');
                        resolve([]);
                    }
                });
                return;
        ...
        let evoWatcher = null;
        const clearEvoWatcher = () => {
            if (evoWatcher) {
                clearInterval(evoWatcher);
                evoWatcher = null;
            }
        };
        const triggerEvoSearch = (api, rawValue) => {
            const value = (rawValue || '').toString().trim();
            if (evoState.timer) {
                clearTimeout(evoState.timer);
            }
            if (value.length < evoSettings.minChars) {
                evoState.searchItems = [];
                evoState.searchMap = {};
                evoState.updating = true;
                api.setData({ evo_results: [] });
                evoState.updating = false;
                return;
            }
            evoState.timer = setTimeout(() => {
                fetchEvoSearch(value).then((results) => {
                    const items = [];
                    const map = {};
                    ...
                    evoState.searchMap = map;
                    evoState.updating = true;
                    api.setData({ evo_results: items });
                    evoState.updating = false;
                });
            }, evoSettings.debounce);
        };
        const attachEvoWatcher = (api) => {
            if (!evoEnabled || evoWatcher) {
                return;
            }
            let lastValue = (api.getData().evo_search || '').toString();
            evoWatcher = setInterval(() => {
                const current = (api.getData().evo_search || '').toString();
                if (current === lastValue) {
                    return;
                }
                lastValue = current;
                triggerEvoSearch(api, current);
            }, 250);
        };
        const buildDialogSpec = (dataOverride) => {
            const body = makeDialogBody(urlInput, buildEvoSearchItems(), displayText, titleText, catalogs, settings.hasUploadPanel, getDocumentsFileTypes(editor));
            const spec = {
                ...
                initialData: dataOverride || initialData,
                onChange: (api, details) => {
                    if (evoState.updating) {
                        return;
                    }
                    ...
                    if (name === 'evo_search') {
                        const rawValue = details.value !== undefined ? details.value : api.getData().evo_search;
                        triggerEvoSearch(api, rawValue);
                        return;
                    }
                ...
                onAction: (api, details) => {
                    if (details.name !== 'evo_results') {
                        return;
                    }
                    const selected = details.value ? evoState.searchMap[String(details.value)] : null;
                    if (!selected) {
                        return;
                    }
                    const href = buildEvoHref(selected);
                    if (!href) {
                        return;
                    }
                    const data = api.getData();
                    const meta = data.url && data.url.meta ? data.url.meta : {};
                    const title = selected.title || selected.pagetitle || '';
                    data.url = {
                        value: href,
                        meta: Object.assign({}, meta, { text: title, title })
                    };
                    data.link = href;
                    data.anchor = href;
                    if (!data.text && title) {
    ...
    const open = (editor) => {
        ...
        data.then((info) => {
            ...
            const dialogApi = editor.windowManager.open(result.spec);
            result.attachEvoWatcher(dialogApi);
        });
    };
